...
for (i = 1; i < n; i++)
/* parent creates all children*/
if ((childpid = fork()) <= 0)
break;
if (childpid > 0) {
/* write synchronization characters to pipe*/
for (i = 0; i < n; i++)
...
fprintf(stderr, "i:%d process ID:%ld parent ID:%ld child ID:%ld Buffer:%s\n",
i, (long)getpid(), (long)getppid(), (long)childpid, buf);
return (childpid == -1);
}
